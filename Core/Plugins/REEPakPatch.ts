...
export function ProcessDirectory(directory: string, outputFile: string = "MyMod.pak") {
    ...
    if (fs.existsSync(folder) && fs.statSync(folder).isDirectory()) {
        ...
        sortedFiles.forEach(obj => {
            ...
            let text: string = obj.split("\\").join("/").split(baseDirectory).join("")
            if (text.startsWith("/")) {
                text = text.slice(1)
            }
            
            let hash: number = MurMurHashV3(text.toLowerCase(), 4294967295)
