...
export abstract class REEngine {
    private static IsFilePatchPak(file: string): boolean {
        return (
            file.includes('re_chunk_') &&
            file.includes('pak.patch') &&
            file.includes('.pak')
        );
    }

    private static FixPatchPakFileName(value: number): string {
        return 're_chunk_000.pak.patch_<REPLACE>.pak'.replace(
            '<REPLACE>',
            value.toString(),
        );
    }
    ...
    public static IsValidPatchPak(directory: string): boolean {
        if (path.extname(directory) == '.pak') {
            if (
                REEngine.IsFilePatchPak(directory) ||
                directory.includes('re_chunk_000')
            ) {
                return false;
            }
    ...
    public static HasValidPatchPaks(mod: IModData): boolean {
    ...
    public static Patch(list: Array<IModData>): Array<IModData> {
        let startIndex: number = 2;
        const patchList: Array<IModData> = list;
        ...
        patchList.forEach(mod => {
            mod.Files.forEach(file => {
                if (
                    path.extname(file.SourcePath) == '.pak' &&
                    !REEngine.IsValidPatchPak(file.SourcePath) &&
                    !file.SourcePath.includes('re_chunk_000') &&
                    mod.IsEnabled == true
                ) {
                    const patchedPath: string = file.InstallPath.replace(
                        path.basename(file.InstallPath),
                        REEngine.FixPatchPakFileName(startIndex),
                    );
